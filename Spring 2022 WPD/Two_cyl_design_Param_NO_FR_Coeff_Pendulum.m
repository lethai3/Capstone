...
% Delete the next three before running the batch script
rad_Crank=0.05;
Orifice_diam=0.04;
FR_coeff=pi*(Orifice_diam./2).^2; %[in^2]

%% Pendulum parameters
Axle_Diam= 1.0; %[m]
Axle_radius=Axle_Diam/2;
...
%rad_Spool=(3*0.1520/4); %[m] spool radius ~4.5 in
rad_Spool=0.1016/2; %[m] spool radius 2 in
%rad_Spool=0.254/2; %[m] spool radius 5 in
%%%%%%%%%%rad_Crank= 0.75*(0.1593/2); %[m] crank radius
%rad_Crank= 0.10; %[m] crank radius
%rad_Crank= 0.09; %[m] crank radius
%rad_Crank= 0.08; %[m] crank radius ****
%rad_Crank= 0.07; %[m] crank radius
%rad_Crank= 0.06; %[m] crank radius
%rad_Crank= 0.05; %[m] crank radius
%rad_Crank= 0.04; %[m] crank radius
%rad_Crank= 0.03; %[m] crank radius
